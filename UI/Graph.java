...
final static int ttt = 3;
    ...
    void rightDrawString(
            ...
            int y) {
    TextLayout layout =
        new TextLayout(s, parent.normalFont, g.getFontRenderContext());
    ...
    synchronized (parent.parent) {
        ...
        drawGraph(clipped, parent.moteListModel.get(i),ttt);
        }
    }
    }
    ...
    protected void drawGraph(Graphics g, int nodeId,int total) {
    SingleGraph sg = new SingleGraph(g, nodeId,0,total);
    SingleGraph sgh = new SingleGraph(g, nodeId,1,total);
    SingleGraph sgl = new SingleGraph(g, nodeId,2,total);

    if (gx1 - gx0 >= width) {
        for (int sx = 0; sx < width; sx++){
        sg.nextPoint(g, graphX(sx), sx);
        sgh.nextPoint(g, graphX(sx), sx);
        sgl.nextPoint(g, graphX(sx), sx);}
    } else {
        for (int gx = gx0; gx <= gx1; gx++){
        sg.nextPoint(g, gx, screenX(gx));
        sgh.nextPoint(g, gx, screenX(gx));
        sgl.nextPoint(g, gx, screenX(gx));}
    }
    }
    ...
    int lastsx, lastsy, nodeId,attribute,totalAtt;

    /* Start drawing the graph mote id */
    SingleGraph(Graphics g, int id,int att,int total) {
        nodeId = id;
	attribute=att;
	totalAtt=total;
        lastsx = -1;
    ...
    void nextPoint(Graphics g, int gx, int sx) {
        int gy = parent.parent.data.getData(nodeId, totalAtt*gx+attribute);
        int sy = -1;
    ...
    private void recenter(int max,int mode) {
    ...
    void setScale(int newScale,int mode) {
    gx1 = gx0 + (MIN_WIDTH << newScale);
    scale = newScale;
    recenter(parent.parent.data.maxX(),mode);
    repaint();
    }
